                                ...
                                foreach ($doctors as $row)
                                {
                                    ...
                                    <tr class="odd gradeX">
                                        ...
                                        <td><?= $row->mobile ?></td>
                                         <td>
                                       <?php if($row->status == 1) {  ?>
                                        <button type="submit" value = "<?= $row->status ?>" class="btn btn-success btn-sm br2 fs12" name="status" onclick="statusCheck(this.value,'<?= $row->user_id ?>')">Activated</button>
                                       <?php } else if($row->status == 2) { ?>
                                       <button type="submit" value = "<?= $row->status ?>" class="btn btn-info btn-sm br2 fs12" name="status" onclick="statusCheck(this.value,'<?= $row->user_id ?>')">Deactivated</button>
                                       <?php } else if($row->status == 3) { ?>
                                       <button class="btn btn-warning btn-sm br2 fs12">Rejected</button>
                                       <?php } ?>
                                      </td>
                                        <td><a href="<?php echo base_url(); ?>admin/doctor_view/<?= $row->user_id ?>">
                                        <button type="button" name="user_id" class="btn btn-info"><span class="glyphicon glyphicon-eye-open"></span>View</button>                                      
                                      <a href="<?php echo base_url(); ?>admin/delete_doctor/<?= $row->user_id ?>"><button type="button" class="btn btn-danger"><span class="glyphicon glyphicon-trash"></span></button></a></td>
                                      
                                      <td>
                                          <form method="post" action="<?php echo base_url(); ?>/admin/update_fees">
                                              <input type="number" name="dr_fees"  value="<?= $row->dr_fees ?>" required/>
                                              <input type="hidden" name="user_id"  value="<?= $row->user_id ?>" required/>
                                              <input class="btn btn-default" type="submit" value="Change Fees">
                                             </form>
                                      </td>
                                    </tr>
                                    <?php                                   
...
if(confirm("are you sure you want to activate or deactivate this user ?"))
  {
   $.ajax({
    url:'<?php echo base_url('admin/active_deactive_user'); ?>',
    type:'POST',
