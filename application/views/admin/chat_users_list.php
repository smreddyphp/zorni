                                ...
                                foreach ($free_dentals as $row)
                                {
                                    ...
                                    <tr class="odd gradeX">
                                        ...
                                        <td><?= $row->mobile ?></td>
                                         <td>
                                       <?php if($row->status == 1) {  ?>
                                        <button type="submit" value = "<?= $row->status ?>" class="btn btn-success btn-sm br2 fs12" name="status" >Deactivate</button>
                                       <?php } else if($row->status == 2) { ?>
                                       <button type="submit" value = "<?= $row->status ?>" class="btn btn-info btn-sm br2 fs12" name="status" >Activate</button>
                                       <?php } else if($row->status == 3) { ?>
                                       <button class="btn btn-warning btn-sm br2 fs12">Rejected</button>
                                       <?php } ?>
                                      </td>
                                    <td>
                                        <a href="<?php echo base_url().'admin/free_dental_chat_view/'.@$dental_id.'/'.@$row->user_id; ?>">
                                            <button type="button" class="btn btn-info"><span class="glyphicon glyphicon-eye-open"></span> View</button>
                                        </a></td>
                                    </tr>
                                    <?php                                   
                                }
